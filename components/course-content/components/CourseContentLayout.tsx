...
export const CourseContentLayout = ( { courseId, courseTitle }: { courseId?: string, courseTitle?: string; } ) => {
  ...
  const { courseSectionDelete, isPending: isDeletePending } = useDeleteCourseSection();
  const { updateSection, isPending: isUpdatePending } = useUpdateCourseSection();
  ...
  const handleDragEnd = async ( event: DragEndEvent ) => {
    ...
    if ( over && active.id !== over.id ) {
      const activeSection = courseSections.find( section => section.id === active.id );
      const overSection = courseSections.find( section => section.id === over.id );
        ...
        <UI.Card>
          <UI.CardBody className="flex items-center justify-center min-h-64">
            <div className="text-center">
              <Icons.IoInformationCircleOutline className="text-5xl text-default-300 mx-auto mb-4" />
              <p className="text-default-500">
                Selecciona una sección para ver detalles o arrastra para reordenar
              </p>
            </div>
          </UI.CardBody>
        </UI.Card>
